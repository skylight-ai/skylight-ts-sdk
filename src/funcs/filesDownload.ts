...
export function filesDownload(
  ...
  Result<
    components.FileDownloadResponse,
...
> {
  const input: operations.DownloadRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.DownloadRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/windows/files/download/{instance_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.FileDownloadResponse$inboundSchema),
